...
ticker.labs <- c("XLK: Technology", "XLF: Financial", "XLV: Health Care", 
                 "XLY: Consumer Discretionary", "XLE: Energy", "XLU: Utilities")
names(ticker.labs) <- c("XLK.Adjusted", "XLF.Adjusted", "XLV.Adjusted",
                        "XLY.Adjusted", "XLE.Adjusted", "XLU.Adjusted")
...
# Join dataframe
war_ticker_inflation_df <- left_join(war_ticker_df, inflation_df, by=c('year_month'='TIME'))
```
...
```{r, include=FALSE}
# Create ticker list
ticker_cols <- colnames(war_ticker_inflation_df)[3:8]

# Create matrix to store p.Values of Granger-Test
var_ticker <- matrix(NA, ncol = 2, nrow = 6)
colnames(var_ticker) <- c('p.Value','p.Value.reverse')
rownames(var_ticker) <- colnames(war_ticker_df)[3:8]
...
Only the Consumer Discretionary Select Sector seems to be influenced by the crisis measure. It is the only index with a significant p-value for the Granger test.
